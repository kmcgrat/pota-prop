...
class RegionalLightningSummary:
    ...
    def get_activity_level(self) -> LightningActivityLevel:
        ...
        if d <= 15.0:
            ...
            level = 5
        elif d <= 400.0 or count >= 10:
            level = 4
        elif d <= 550.0 or count >= 5:
            level = 3
        elif d <= 750.0 or count >= 1:
            level = 2
        else:
            level = 1

        # Density boost: 5+ active cells / clusters within 150 mi
        nearby_count = sum(1 for c in self.storm_cells if c.distance_miles <= 150.0)
        if nearby_count >= 5 and level < 9:
            level = min(9, level + 1)
        ...
        level_configs = {
            1: (
                ...
                "Clear",
                "1",
                "Normal operating conditions. All bands clear.",
                "No active convective storm cells detected within 750 miles.",
                False,
            ),
            2: (
                "#3fb950",
                "Very Low",
                "2",
                "Distant storms. Bands clear.",
                "Distant storm cells (550–750 mi). Negligible background sferics.",
                False,
            ),
            3: (
                "#7ee787",
                "Low",
                "3",
                "Low background sferics. Minor static on 80m/160m.",
                "Distant storm clusters (400–550 mi). Minor low-band noise — 20m and above unaffected.",
                False,
            ),
            4: (
                "#d29922",
                "Moderate",
                "4",
                "Noticeable static crashes on 40m/80m. Weak DX signals may be affected.",
                "Regional storm clusters (250–400 mi). Elevated hash on lower HF bands.",
                False,
            ),
            5: (
                "#db6d28",
                "Elevated",
                "5",
                "Elevated noise floor on 40m/80m. Consider higher bands if available.",
                "Active thunderstorms within 150–250 mi. Noise floor elevated on 40m/80m.",
                False,
            ),
            6: (
                "#f0883e",
                "Notable",
                "6",
                "Frequent static crashes. Monitor storm movement.",
                "Thunderstorms active within 100–150 mi. Noticeable QRN on 40m/80m; 20m may have some hash.",
                False,
            ),
            7: (
                "#e06c3a",
                "Storms Nearby",
                "7",
                "Heavy QRN on lower bands. Consider higher bands or shorter sessions.",
                "Active thunderstorm clusters within 60–100 mi. S5–S7 static on low/mid bands.",
                False,
            ),
            8: (
                "#da3633",
                "Close Storms",
                "8",
                "Heavy local QRN. Storms approaching your area — watch conditions closely.",
                "Thunderstorms within 30–60 mi. Heavy S7–S9 static crashes across lower HF bands.",
                False,
            ),
            9: (
                "#f85149",
                ...
                "9",
                "⚠️ WARNING: Lightning within 30 miles! Consider disconnecting antenna feedlines and rotor cables.",
                "Thunderstorms within 15–30 mi. S9+ static. Risk of electrostatic induction.",
                True,
            ),
            10: (
                "#ff2a55",
                "Immediate Hazard",
                "10",
                "🚨 DANGER: Lightning in immediate vicinity (< 15 miles)! Disconnect all antennas and unplug rigs now.",
                "Lightning strikes in immediate vicinity (< 15 mi). S9+20 to S9+40 static crashes.",
                True,
            ),
        }

        color, label, short_text, advisory, description, is_disc = level_configs[level]
        return LightningActivityLevel(
            level=level,
            color=color,
            label=label,
    ...
    def format_tooltip_html(self) -> str:
        ...
        if closest_nws:
            ...
            w_age_str = f" (issued {w_age}m ago)" if w_age > 0 else ""
            w_strikes = closest_nws.actual_strikes_in_polygon
            w_rate = max(1, int(round(w_strikes / 15.0))) if w_strikes > 0 else (5 if w_age <= 30 else 1)
            lines.append(
                ...
                f"<span style='color: #8b949e; font-size: 11px; font-weight: normal;'>{w_age_str}</span></div>"
            )
            lines.append(
        ...
        live_clusters = [
            c for c in self.storm_cells
            if c.is_live_cluster or "Live" in c.event_type or c.total_strikes_in_cluster > 0
        ]
        if live_clusters:
            live_clusters.sort(key=lambda c: c.distance_miles)
            lines.append(
            ...
            for cluster in live_clusters[:2]:
...
class StrikeBuffer:
    ...
    def cluster_strikes(
        ...
        for c in clusters:
            ...
            rate_per_min = max(1, int(round(recent_15m / 15.0))) if recent_15m > 0 else 1
            weight = max(0.5, min(3.0, len(c) / 10.0))
...
class BlitzortungStreamThread(threading.Thread):
    ...
    def _connect_and_stream(self, host: str):
        ...
        req = (
            ...
            f"User-Agent: POTA-Hunter/26.8.5\r\n\r\n"
        )
        ...
        while self._running:
            try:
                ...
                # Send heartbeat keepalive every 25s if quiet
                if time.time() - last_ping_time > 25.0:
                    ssock.sendall(make_ws_frame(json.dumps({"a": 111})))
                    last_ping_time = time.time()

            except socket.timeout:
                # Timeout is normal — send ping/subscription keepalive
                try:
                    ssock.sendall(make_ws_frame(json.dumps({"a": 111})))
                    last_ping_time = time.time()
                except Exception:
...
class LightningEngine:
    ...
    def _fetch_nws_warnings_if_needed(self, home_lat: float, home_lon: float, timeout: int = 5) -> List[NWSWarning]:
        ...
        try:
            req = urllib.request.Request(
                url,
                headers={
                    "User-Agent": "POTA-Hunter-Propagation-Engine/26.8.5",
    ...
    def _compute_hybrid_summary(self, home_lat: float, home_lon: float, timeout: int = 5) -> RegionalLightningSummary:
        """
        Generates comprehensive summary combining NWS warning baseline and
        real-time Blitzortung WebSocket strikes with polygon intersection counting.
        """
        ...
        # Decide whether we are in initial bootstrap phase or full live stream phase
        # If stream has been active < 45s and has few strikes, synthesize NWS bootstrap
        if (len(all_live_strikes) < 3 and stream_uptime < 45.0) or not is_stream_live:
            # Bootstrap using NWS active warnings
            storm_cells: List[StormCell] = []
            for w in nws_warnings:
                ...
                base_weight = 1.5 if "Tornado" in w.event_type else 1.0
                weight = round(base_weight * age_factor, 2)
                rate_min = max(1, int(round(20.0 * weight)))
                strikes_15m = max(w.actual_strikes_in_polygon, rate_min * 15)

                if w.polygon_coords:
                    avg_lon = sum(c[0] for c in w.polygon_coords) / len(w.polygon_coords)
                    avg_lat = sum(c[1] for c in w.polygon_coords) / len(w.polygon_coords)
                else:
                    avg_lat, avg_lon = home_lat, home_lon
                ...
                storm_cells.append(
                    StormCell(
                        ...
                        estimated_strikes_per_min=rate_min,
                        estimated_strikes_15m=strikes_15m,
                        alert_age_minutes=w.issued_minutes_ago,
                        actual_strikes_in_polygon=w.actual_strikes_in_polygon,
                    )
                )

            # Add any live clusters if available
            storm_cells.extend(live_clusters)
            storm_cells.sort(key=lambda s: s.distance_miles)

            total_rate = sum(c.estimated_strikes_per_min for c in storm_cells)
            total_15m = sum(c.estimated_strikes_15m for c in storm_cells)
            source_desc = "NOAA NWS / Convective Radar"
        else:
            # Full Real-Time Blitzortung Streaming Phase
            storm_cells = list(live_clusters)
            # If NWS warnings exist in areas without live strikes, also preserve them with live polygon strike counts
            for w in nws_warnings:
                has_overlapping_cluster = any(
                    abs(c.distance_miles - w.distance_miles) < 25.0 for c in storm_cells
                )
                if not has_overlapping_cluster:
                    if w.polygon_coords:
                        avg_lon = sum(c[0] for c in w.polygon_coords) / len(w.polygon_coords)
                        avg_lat = sum(c[1] for c in w.polygon_coords) / len(w.polygon_coords)
                    else:
                    ...
                    storm_cells.append(
                        StormCell(
                            latitude=avg_lat,
                            longitude=avg_lon,
                            intensity_weight=1.0 if "Tornado" not in w.event_type else 1.5,
                            event_type=w.event_type,
                            headline=w.headline,
                            distance_miles=w.distance_miles,
                            bearing_deg=w.bearing_deg,
                            estimated_strikes_per_min=max(1, int(round(w.actual_strikes_in_polygon / 15.0))) if w.actual_strikes_in_polygon > 0 else 5,
                            estimated_strikes_15m=w.actual_strikes_in_polygon,
                            alert_age_minutes=w.issued_minutes_ago,
                            actual_strikes_in_polygon=w.actual_strikes_in_polygon,
                        )
                    )
            ...
            storm_cells.sort(key=lambda s: s.distance_miles)
            total_rate = max(1, int(round(count_15m / 15.0))) if count_15m > 0 else (len(storm_cells) * 2 if storm_cells else 0)
